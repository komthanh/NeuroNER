...
def predictionAPI(sess, dataset, dataset_type, model, transition_params_trained, stats_graph_folder, epoch_number,
               parameters, dataset_filepaths):
    """
    Tagging based on a pre-trained model. Then output the raw prediction to be evaluated by an external application.
    :param sess:
    :param dataset:
    :param dataset_type:
    :param model:
    :param transition_params_trained:
    :param stats_graph_folder:
    :param epoch_number:
    :param parameters:
    :param dataset_filepaths:
    :return: Sequences of raw predicted labels.
    """
    if dataset_type == 'deploy':
        print('Predict labels for the {0} set'.format(dataset_type))
    else:
    ...
    return all_predictions, output_filepath
